...
- After the text of every original article is final and the user has uploaded its reference photo, use `scene-distillation-zine-v1-3` with that photo to extract one memorable visual metaphor, then generate one article-specific, transparent-background PNG header illustration. Do not substitute an unrelated stock image or generate before receiving the reference photo. Depict one simple isolated object and compose it as a square image displayed at 240 × 240 px on desktop. Remove most source detail and favor aged watercolor, dry-print, low-poly, or restrained tiny-pixel character over polished vectors or photorealism. Preserve meaningful natural colors when they give the subject more character; the site's purple is optional rather than mandatory. Do not include text in the artwork. Save the PNG under `assets/img/blog/`, set `illustration` and localized `illustration_alt` frontmatter in both language versions, and reuse the same image for the bilingual pair.
- In the homepage `Latest Posts` list, place each post's frontmatter `illustration` immediately before its title as a decorative square icon. Size the icon to `2em` (twice the title text height), center it vertically on the title text, and keep the title text aligned to the date by its text baseline. Position the icon outside normal layout flow so it must not increase the row height, alter the original vertical padding, or move the horizontal divider lines. The icon and title should remain part of the same clickable link.
- Begin every blog post with one compact metadata line in this order: `Month DD, YYYY · N views · N min`. Do not prefix the date with `Created on`. The views value must follow the unified bilingual analytics rule above, and the reading-time value must use the automatic calculation below. Use the same English-style date and `min` label in both language versions.
