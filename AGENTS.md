...
- Treat each Chinese-English article pair as one page for analytics. Set the same `view_count_path` in both versions so GoatCounter reporting and displayed view totals always share one count. For an existing pair whose language URLs already have separate counts, query both and use the path with the higher count as the shared value. For a new pair, use the primary-language article URL from the first publication.
